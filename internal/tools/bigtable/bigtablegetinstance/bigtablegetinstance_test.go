...
func TestToolConfigType(t *testing.T) {
	config := bigtablegetinstance.Config{}
	if got := config.ToolConfigType(); got != "bigtable-get-instance" {
		t.Errorf("ToolConfigType() = %v, want bigtable-get-instance", got)
	}
}
